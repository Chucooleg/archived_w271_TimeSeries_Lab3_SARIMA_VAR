...
(Trend clearly not stationary --> growing variance. can be random walk with mild drift)
...
(Less non-linear than the monthly series. Elimination of seasons make overall trend correlation clearer but lost the granularity. we can say much about monthly lags of one series on another now.)
...
```{r}
po.test(cbind(auto.intersect.ts, unem.intersect.ts))
```
